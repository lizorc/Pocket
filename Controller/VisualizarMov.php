<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // SELECT Account, SUM(Value) FROM Movement WHERE MONTH(Date) = 5 AND Control = "Ingreso" AND Name_User = "Juan Miguel" GROUP BY Account;

        // Consult Password in the Table 
        $SQL = "SELECT * FROM Movement WHERE MONTH(Date) = '$Month' AND Name_User = '$username'";
        $Result = $connection -> query($SQL);

        $SQL1 = "SELECT Account, SUM(Value) FROM Movement WHERE MONTH(Date) = '$Month' AND Control = 'Ingreso' AND Name_User = '$username' GROUP BY Account";
        $Result1 = $connection -> query($SQL1);

        $SQL2 = "SELECT Account, SUM(Value) FROM Movement WHERE MONTH(Date) = '$Month' AND Control = 'Egreso' AND Name_User = '$username' GROUP BY Account";
        ...
        FROM Movement WHERE MONTH(Date) = '$Month' AND Name_User = '$username' GROUP BY Account";
